...
<head>
    ...
    <script>
        ...
        $(function() {
            ...
            function list() {
                ...
                $.get('fm.php?do=list&file=' + hashval, function(data) {
                    $tbody.empty();
            ...
            function renderFileRow(data) {
                var $link = $('<a class="name" />')
                    ...
                    .attr('onClick', !data.is_dir ? 'pagecall(this)' : '')
                    .text(data.name);
                var allow_direct_link = <?php echo $allow_direct_link ? 'true' : 'false'; ?>;
                if (!data.is_dir && !allow_direct_link) $link.css('pointer-events', 'none');
                var $dl_link = $('<a/>').attr('href', 'fm.php?do=download&file=' + encodeURIComponent(data.path))
    ...
    <div id="top">
        <?php if ($allow_create_folder) : ?>
            <form action="fm.php?" method="post" id="mkdir" />
            <label for=dirname>Create New Folder</label>
            <div class="form-group">
                <input id=dirname type=text name=name value="" class="form-control" />
